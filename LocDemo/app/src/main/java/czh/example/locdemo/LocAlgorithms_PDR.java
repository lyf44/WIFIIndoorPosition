...
public class LocAlgorithms_PDR extends LocAlgorithms{
	...
	public Position[] DoLoc(StepDetectionParameters stepDetectionParameters, Position WIFIPosition, Position[] particle, Boolean flag_WIFIchanged) {
		...
		if(flag_WIFIchanged) {
			float z_R = 0.5f; // vairance of x and y
			double sum_P_w = 0;
			...
			for (i = 0; i < 500; i++) {
				...
				//3D normal distribution, with covariance=0.
			...
			for (i = 0; i < 400; i++) {
				...
				//resample from cumulative distributuon function
				for (j = 0; j < 500; j++) {
			...
			//else if wifi signal is not found, new update is performed
			for(i=1;i<500;i++){
